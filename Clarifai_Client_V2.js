// clarifai_node.js - the Clarifai client API for node.js programs - V2
// Require the client
 ...
 */ 
Clarifai.prototype._commonHttpStatusHandler = function(  res, responseData, resultHandler, retry ) {
...
function processResponse(bestColor, resultHandler) {
	if (bestColor) {
		resultHandler(null, bestColor);
	}
}
Clarifai.prototype._colorDetect = function(urls, resultHandler, retry) {
    ...
    function(response) {
    	...
    	function compareNumbersByMax(a, b) {
		  	return b.max_value - a.max_value;
		}
		function compareNumbersByCount(a, b) {
    	...
    	if (status.description == 'Ok') {
    		...
    		var colors = response.rawData.outputs;
    		var listed_color = [];
    		if (colors.length > 1) {
    			console.log("P1");
    			for (var i = 0; i <= colors.length - 1; i++) {
    				...
    				var itemData = colorItem.data.colors;
    				for (var j = 0; j <= itemData.length - 1; j++) {
    					
    					var item = itemData[j];
    					var flag_not_exist = false;
    					if (listed_color.length > 0) {
    						for (var k = 0; k <= listed_color.length - 1; k++) {    						
	    						var kItem = listed_color[k];
	    						
	    						if (kItem.item.raw_hex == item.raw_hex) {    
	    							// console.log("P5");	
	    							// console.log("kItem:",kItem, "original count:",kItem.count, "original max:",kItem.max_value);						
	    							kItem.count ++;	    							
									var current_Max = kItem.max_value;
									kItem.max_value = Math.max(current_Max, item.value);	    							
									// console.log("kItem:",kItem, "plused count:",kItem.count, "changed max", kItem.max_value);
	    							flag_not_exist = true;
	    							break;
	    						} 
    					...
    					if (flag_not_exist == false) {
    						// console.log("P6");
    						// console.log("item", {item: item, count: 1, max_value:item.value});
    						listed_color.push({item: item, count: 1, max_value:item.value});
    					}
    				}
    			}
    		...
    		var colorDataByMax = temp1.sort(compareNumbersByMax);
    		// console.log("sorted colors by max:", colorDataByMax);
    		var temp2 = listed_color;
    		var colorDataByCount = temp2.sort(compareNumbersByCount);
    		// console.log("sorted colors by count:", colorDataByCount);

    		var bestColor;
    		// var bestMatchesbyMax = colorDataByMax.slice(0,3);
    		// var bestMatchesbyCount = colorDataByCount.slice(0,3);
    		console.log("Best Color by Max:", colorDataByMax, "Best Color by Count:", colorDataByCount);
    		if (colorDataByMax.length < 3) {
    			console.log("Case 1");
    			bestColor = colorDataByMax[0];
    			processResponse(bestColor, resultHandler);
    		} else {
    			var bm0 = colorDataByMax[0], bm1 = colorDataByMax[1], bm2 = colorDataByMax[2];
    			var bc0 = colorDataByCount[0], bc1 = colorDataByCount[1], bc2 = colorDataByCount[2];
    			console.log("bm0:hex: ", bm0.item.raw_hex, "bc0:hex:",bc0.item.raw_hex);
    			if (bm0.item.raw_hex === bc0.item.raw_hex) {
    				console.log("bm0:hex: ", bm0.item.raw_hex, "bc0:hex:",bc0.item.raw_hex);		
    				console.log("Case 2");
    				bestColor = bm0;
    				processResponse(bestColor, resultHandler);
    				...
    				if (bm0.count >= 2) {
    					...
    					if (bc0.count == 1) {
    						...
    						processResponse(bestColor, resultHandler);
    						return;
    					} else if (bc0.count == 2 && bc0.max_value >= 0.6 * bm0.max_value) {
    						console.log("Case 5");
    						bestColor = bc0;
    						processResponse(bestColor, resultHandler);
    						return;
    					} else if (bc0.count == 3 && bc0.max_value >= 0.2 * bm0.max_value) {
    						console.log("Case 6");
    						bestColor = bc0;
    						processResponse(bestColor, resultHandler);
    						return;
    						...
    						if (bc0.count == 2) {
    							if (bm1.count == 2 && bm1.max_value >= 0.6 * bm0.max_value) {
    								console.log("Case 7");
    								bestColor = bm1;
    								processResponse(bestColor, resultHandler);
    								return;
    							} else if (bm2.count == 2 && bm2.max_value >= 0.6 * bm0.max_value) {
    								console.log("Case 8");
    								bestColor = bm2;
    								processResponse(bestColor, resultHandler);
    								return;
    							} else if (bc1.count == 2 && bc1.max_value >= 0.6 * bm0.max_value) {
    								console.log("Case 9");
    								bestColor = bc1;
    								processResponse(bestColor, resultHandler);
    								return;
    							} else if (bc2.count == 2 && bc2.max_value >= 0.6 * bm0.max_value) {
    								console.log("Case 10");
    								bestColor = bc2;
    								processResponse(bestColor, resultHandler);
    								return;
    							...
    							if (bm1.count == 3 && bm1.max_value >= 0.2 * bm0.max_value) {
    								console.log("Case 12");
    								bestColor = bm1;
    								processResponse(bestColor, resultHandler);
    								return;
    							} else if (bm2.count == 3 && bm2.max_value >= 0.2 * bm0.max_value) {
    								console.log("Case 13");
    								bestColor = bm2;
    								processResponse(bestColor, resultHandler);
    								return;
    							} else if (bc1.count == 3 && bc1.max_value >= 0.2 * bm0.max_value) {
    								console.log("Case 14");
    								bestColor = bc1;
    								processResponse(bestColor, resultHandler);
    								return;
    							} else if (bc2.count == 3 && bc2.max_value >= 0.2 * bm0.max_value){
    								console.log("Case 15");
    								bestColor = bc2;
    								processResponse(bestColor, resultHandler);
    								return;
    							} else if (bm1.count == 2 && bm1.max_value >= 0.6 * bm0.max_value) {
    								console.log("Case 16");
    								bestColor = bm1;
    								processResponse(bestColor, resultHandler);
    								return;
...
Clarifai.prototype.colorDetect = function(urls, callback) {
	retry = function() {
	  this.colorDetect( urls, callback );
	}.bind(this);
	this._colorDetect(urls, callback, retry);
}
