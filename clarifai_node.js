...
Clarifai.prototype._colorURL  = function( url, localId, resultHandler, retry ) {
	if( this._throttled ) 
		// the host has throttled us, so there's no point in sending the request
		// just immediately return the throttled status
		resultHandler( { 'status_code': 'ERROR_THROTTLED',
	...
	this._httpRequest( colorPath, form, localId, resultHandler, retry );

}

Clarifai.prototype.colorURL = function( url, localId, callback ) {

  retry = function() {
      this.colorURL( url, localId, callback );
  }.bind(this);

  this._colorURL( url, localId,
    callback,	
    retry );
